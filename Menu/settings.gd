...
func _ready() -> void:
	...
	$Control/HBoxContainer/CheckButton.set_pressed(save_system.fullscreen_enabled)
...
func _on_h_slider_value_changed(value: float) -> void:
	if slider_block:
		return
	# Ajusta volume de efeitos sonoros (bus 1)
	MusicController.volume_bus_1 = value
...
func _on_h_slider_2_value_changed(value: float) -> void:
	if slider_block:
		return
	# Ajusta volume de música de fundo (bus 2)
	Effects.volume_bus_2 = value
	Effects.apply_sfx_volume()
