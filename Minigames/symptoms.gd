...
@onready var illness_list = $Manual/ManualContent/MarginContainer/VBoxContainer/PageContent/IllnessList
@onready var prev_page_button = $Manual/ManualContent/MarginContainer/VBoxContainer/PaginationButtons/PrevPageButton
@onready var next_page_button = $Manual/ManualContent/MarginContainer/VBoxContainer/PaginationButtons/NextPageButton
@onready var page_label = $Manual/ManualContent/MarginContainer/VBoxContainer/PaginationButtons/PageLabel
...
var available_illnesses: Array = [
	...
	{"name": "Asthma Attack", "symptoms": ["Shortness of breath", "Wheezing", "Chest tightness", "Cough"], "vitals": {"hr": [90, 130], "spo2": [85, 95]}},
	{"name": "Diabetes Crisis", "symptoms": ["Confusion", "Dizziness", "Nausea", "Excessive thirst"], "vitals": {"hr": [80, 120], "spo2": [90, 100]}},
	{"name": "Septic Shock", "symptoms": ["Fever", "Rapid breathing", "Confusion", "Low blood pressure"], "vitals": {"hr": [120, 160], "spo2": [80, 90]}},
	{"name": "Kidney Infection", "symptoms": ["Fever", "Back pain", "Nausea", "Frequent urination"], "vitals": {"hr": [85, 115], "spo2": [90, 100]}},
	{"name": "Gastroenteritis", "symptoms": ["Abdominal pain", "Nausea", "Diarrhea", "Fever"], "vitals": {"hr": [75, 105], "spo2": [95, 100]}},
	{"name": "Meningitis", "symptoms": ["Severe headache", "Fever", "Stiff neck", "Confusion"], "vitals": {"hr": [90, 130], "spo2": [90, 100]}},
	{"name": "Pulmonary Embolism", "symptoms": ["Chest pain", "Shortness of breath", "Rapid breathing", "Cough"], "vitals": {"hr": [100, 140], "spo2": [80, 90]}},
	{"name": "Panic Attack", "symptoms": ["Chest pain", "Shortness of breath", "Dizziness", "Rapid heartbeat"], "vitals": {"hr": [100, 150], "spo2": [95, 100]}},
	{"name": "Dehydration", "symptoms": ["Dizziness", "Confusion", "Dry mouth", "Excessive thirst"], "vitals": {"hr": [90, 130], "spo2": [90, 100]}},
	{"name": "Hypothermia", "symptoms": ["Shivering", "Confusion", "Dizziness", "Slow breathing"], "vitals": {"hr": [50, 80], "spo2": [90, 100]}}
]

# New mapping of symptoms to potential illnesses
var symptoms_to_illnesses: Dictionary = {
	"Chest pain": ["Heart Attack", "Pneumonia"],
	"Shortness of breath": ["Heart Attack", "Pneumonia"],
...
var available_treatments: Array = [
	{
		"name": "Antibiotics",
		"good_for": ["Pneumonia", "Kidney Infection", "Meningitis", "Appendicitis"],
	...
	},
	{
		...
		"description": "Medication that relaxes lung muscles"
	},
	{
		"name": "Anticoagulants",
	...
	},
	{
		...
		"description": "Fever reducers and pain relievers"
	},
	{
	...
	},
	{
		...
		"description": "Medication to control nausea and vomiting"
	},
	{
		"name": "Insulin",
		"good_for": ["Diabetes Crisis"],
		"contraindications": ["Low blood sugar", "Dehydration"],
		"description": "Hormone to regulate blood sugar"
	},
	{
		"name": "Antivirals",
		"good_for": ["Common Cold", "Meningitis"],
		"contraindications": ["Kidney problems", "Liver problems"],
		"description": "Medication to treat viral infections"
	},
	{
		"name": "Surgery",
		"good_for": ["Appendicitis"],
		"contraindications": ["Heart Attack", "Stroke"],
		"description": "Surgical intervention for appendicitis"
	},
	{
		"name": "Thrombolytics",
		"good_for": ["Stroke", "Heart Attack"],
		"contraindications": ["Recent surgery", "Bleeding disorders"],
		"description": "Medication to dissolve blood clots"
	},
	{
		"name": "IV Fluids",
		"good_for": ["Dehydration", "Gastroenteritis"],
		"contraindications": ["Heart failure", "Kidney failure"],
		"description": "Intravenous fluid replacement"
	},
	{
		"name": "Warming Treatment",
		"good_for": ["Hypothermia"],
		"contraindications": ["Burns", "Fever"],
		"description": "Gradual warming therapy for hypothermia"
	}
]

# References to HowToTreat UI elements
@onready var how_to_treat = $HowToTreat
@onready var how_to_treat_close_button = $HowToTreat/CloseButton
@onready var how_to_treat_content = $HowToTreat/ManualContent
@onready var treatment_list = $HowToTreat/ManualContent/MarginContainer/VBoxContainer/PageContent/IllnessList
@onready var how_to_treat_prev_page_button = $HowToTreat/ManualContent/MarginContainer/VBoxContainer/PaginationButtons/PrevPageButton
@onready var how_to_treat_next_page_button = $HowToTreat/ManualContent/MarginContainer/VBoxContainer/PaginationButtons/NextPageButton
@onready var how_to_treat_page_label = $HowToTreat/ManualContent/MarginContainer/VBoxContainer/PaginationButtons/PageLabel

# HowToTreat manual state
var is_how_to_treat_expanded = false
var current_treatment_page = 0
var treatments_per_page = 5
...
func _ready():
	...
	print("Close button signal connected: ", manual_close_button.pressed.is_connected(_on_manual_close_pressed))
	...
	illness_list.scroll_active = true
	treatment_list.scroll_active = true
...
func _get_vital_color(value: int, min_normal: int, max_normal: int, is_critical: bool = false) -> Color:
	if is_critical:
...
func _input(event):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		var how_to_treat_close_button_rect = Rect2(how_to_treat_close_button.global_position, how_to_treat_close_button.size)
		...
		print("Is mouse in HowToTreat close button: ", how_to_treat_close_button_rect.has_point(mouse_pos))
		print("=================")
		...
		elif not is_manual_expanded and manual_rect.has_point(mouse_pos):
			_on_manual_clicked()
		elif is_how_to_treat_expanded and how_to_treat_close_button_rect.has_point(mouse_pos):
			_on_how_to_treat_close_pressed()
		elif not is_how_to_treat_expanded and how_to_treat_rect.has_point(mouse_pos):
...
func _process(delta):
	...
	if is_how_to_treat_expanded:
		how_to_treat.position = how_to_treat.position.lerp(manual_expanded_position, delta * manual_animation_speed)
		how_to_treat.scale = how_to_treat.scale.lerp(manual_expanded_scale, delta * manual_animation_speed)
		how_to_treat_content.scale = how_to_treat.scale / manual_normal_scale
	else:
		var mouse_pos = get_viewport().get_mouse_position()
		var how_to_treat_rect = Rect2(how_to_treat.position, how_to_treat.size * how_to_treat.scale)
		
		if how_to_treat_rect.has_point(mouse_pos):
			how_to_treat.scale = how_to_treat.scale.lerp(manual_hover_scale, delta * manual_animation_speed)
		else:
			how_to_treat.position = how_to_treat.position.lerp(Vector2(357.065, 429.0), delta * manual_animation_speed)
			how_to_treat.scale = how_to_treat.scale.lerp(manual_normal_scale, delta * manual_animation_speed)
...
func _update_patient_vitals():
	...
	# Degradation factor: reaches 1 after 60 seconds. Can be adjusted.
	var degradation_factor = clamp(time_elapsed / 60.0, 0, 1)
	...
	var hr_degraded_min = int(lerp(
		float(hr_range[0]), 
		float(HR_CRITICAL[0]), 
		critical_proximity * 1.5  # More dramatic increase
	))
	var hr_degraded_max = int(lerp(
		float(hr_range[1]), 
		float(HR_CRITICAL[1]), 
		critical_proximity * 1.5  # More dramatic increase
	))
	...
	var spo2_degraded_min = int(lerp(
		float(spo2_range[0]), 
		float(SPO2_CRITICAL[0]), 
		critical_proximity * 1.5  # More dramatic decrease
	))
	var spo2_degraded_max = int(lerp(
		float(spo2_range[1]), 
		float(SPO2_CRITICAL[1]), 
		critical_proximity * 1.5  # More dramatic decrease
	))
	...
	var vitals = { 
		"hr": hr, 
		"spo2": spo2,
		"is_critical": patient_health <= 30.0  # Show critical indicators when health is below 30%
	}
...
func _on_manual_clicked():
	...
	# Ensure the manual is on top by moving it to the end of the scene tree
	manual.get_parent().move_child(manual, manual.get_parent().get_child_count() - 1)
	manual.show()
...
func _update_illness_list():
	...
	# Add symptoms and their potential illnesses for current page
	var formatted_text = ""
	...
	illness_list.text = formatted_text
	print("Text updated in illness list")
...
func _update_treatment_page_display():
	how_to_treat_page_label.text = "Page %d/%d" % [current_treatment_page + 1, total_treatment_pages]
...
func _update_treatment_list():
	...
	for i in range(start_idx, end_idx):
		...
		formatted_text += "[b]%s:[/b]\n" % treatment.name
		formatted_text += "Good for: %s\n" % ", ".join(treatment.good_for)
		formatted_text += "Do not use if: %s\n" % ", ".join(treatment.contraindications)
		formatted_text += "Description: %s\n\n" % treatment.description
	
	treatment_list.text = formatted_text
...
func _on_how_to_treat_close_pressed():
	...
	diagnose_button.visible = true
	how_to_treat.scale = manual_normal_scale
	how_to_treat.position = Vector2(357.065, 429.0)  # Reset to original position
	how_to_treat_content.scale = Vector2(1, 1)
...
func _on_how_to_treat_clicked():
	...
	request_info_button.visible = false
	diagnose_button.visible = false
	how_to_treat.get_parent().move_child(how_to_treat, how_to_treat.get_parent().get_child_count() - 1)
	how_to_treat.show()
...
func _show_treatment_options():
	...
	var patient_symptoms = current_patient_data.illness.symptoms
	
	# Find suitable treatments
	var suitable_treatments = []
	for treatment in available_treatments:
		var is_suitable = false
		for illness in treatment.good_for:
			# Check if any of the patient's symptoms match this illness
	...
	# Create buttons for each suitable treatment
...
func _on_treatment_selected(treatment: Dictionary):
	...
	for good_illness in treatment.good_for:
		if good_illness.to_lower() == patient_illness.to_lower():
			...
			break
		elif good_illness.to_lower() in patient_illness.to_lower() or patient_illness.to_lower() in good_illness.to_lower():
			is_correct_treatment = true
	...
	if is_correct_treatment:
		var feedback = "Thank you doctor! I'm starting to feel better already. The %s seems to be helping with my %s." % [
			treatment.name.to_lower(),
			patient_illness.to_lower()
		]
		messages_to_type.append(feedback)
		print("Added positive feedback message")
	else:
		# Check which symptoms are actually helped by this treatment
		var helped_symptoms = []
		...
		if helped_symptoms.size() > 0:
			...
			var feedback_1 = "The %s is helping with my %s, but I still have other symptoms..." % [
				treatment.name.to_lower(),
				random_helped.to_lower()
			]
			...
			var feedback_2 = "The %s doesn't seem to help with my %s." % [
				treatment.name.to_lower(),
				random_symptom.to_lower()
			]
			messages_to_type.append(feedback_1)
